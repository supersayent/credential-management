...
<body style="margin-top: 100px; background-color: #9561e2"><center>
    ...
    <form action="{{ route('user.updateaccount',[$user->id]) }}" method="POST" enctype="multipart/form-data">
        {{ csrf_field() }}
        <input type="text" placeholder="Enter Name" name="name" value="{{ $user->name }}"></br></br>
        <input type="email" placeholder="Enter Email" name="email" value="{{ $user->email }}"></br></br>
        ...
        <input type="text" placeholder="Enter Role" name="role" value="{{ $user->role }}" readonly></br></br>
        <input type="text" placeholder="Enter Status" name="status" value="{{ $user->status }}" readonly></br></br>
